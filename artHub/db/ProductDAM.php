    ...
    public function writeProduct($product) {
        ...
        if ($productDB == null) {
            ...
            $query = 'INSERT INTO products (categoryID, productCode, productName,
                                            description, listPrice, discountPercent)
          VALUES (:categoryID, :productCode, :productName, :description, :listPrice, :discountPercent)';


        $statement = $this->db->prepare($query);
        $statement->bindValue(':categoryID', $product->categoryId);
        ...
        $statement->bindValue(':discountPercent', $product->discountPercent);
        $statement->execute();
        $statement->closeCursor();
        } else {
            // $statement->bindValue(':productName', $product->name);
            ...
            $statement->bindParam(':productCode', $product->productCode, PDO::PARAM_INT);
            $statement->bindParam(':listPrice', $product->listPrice, PDO::PARAM_INT);
            $statement->bindParam(':discountPercent', $product->discountPercent, PDO::PARAM_INT);
            $statement->bindParam(':categoryID', $product->categoryID, PDO::PARAM_INT);
